...
class AgingGAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:           # 1st image in batch----> batch = 2 images
            # Identity loss
            ...
            #print("loss>>>>>>", g_loss)
            writer.add_scalar("Loss/train", g_loss, self.current_epoch)
            writer.flush()
            writer.close()
            ...
            #self.logger.experiment.add_scalar('Real/A', make_grid(self.real_A, normalize=True, scale_each=True),
                                             #self.current_epoch)
            # Log to tb
            if batch_idx % 500 == 0:
                ...
                self.logger.experiment.add_image('Generated/B',
                                                 ...
                                                 self.current_epoch)
                self.logger.experiment.add_scalar("Loss/Train", g_loss,self.current_epoch)
            return output
        ...
        if optimizer_idx == 1:
            ...
            output = {
                'loss': d_loss,
                'log': {'Loss/Discriminator': d_loss, "Accuracy": 60}
            }
            return output
